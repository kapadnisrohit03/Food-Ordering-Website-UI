...
const RestaurantsWithOnlineFoodDelivery = () => {
  ...
  return (
    <div className='restaurantWithOnlineDelivery'>
        <div className='restaurantWithOnlineDelivery-text'>
            <h1 className='font-bold text-2xl'>Restaurants with online food delivery in Mumbai</h1>
        </div>
        <div className='items flex gap-6 flex-wrap mt-[3vw]'>
          {data.map((d)=>{
            return(
              <div key={d.id} className="duration-300 cursor-pointer hover hover:scale-95"
            >
              <div
                className={`flex-shrink-0 w-[17vw] gap-5 relative h-[11vw] `}
              >
                <img
                  src={d.img}
                  alt={d.title}
